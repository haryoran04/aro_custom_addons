...
class connecteur(models.Model):    
    ...
    def update_add_record(self):
        ...
        cursorSQLServer.execute("""SELECT  nxh_date_aro
                                    ...
                                    FROM aro_nexthope_reprise_2 WHERE  nxh_date_nexthope IS NULL """)         
    ...
    def suspension(self,connection,cursorSQLServer,cursorPOSTGRESOdoo,id_version_police):
        if id_version_police:
            ...
            cursorSQLServer.execute("""UPDATE aro_nexthope_reprise_2 SET nxh_date_nexthope =?   WHERE nxh_ver_ident= ? """,(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),id_version_police))
            connection.commit()
    ...
    def traitement_normal(self,id_version_police,id_prime,cursorSQLServer,cursorPOSTGRESOdoo,connection,pol_numpol):
        ...
        if id_version_police and id_prime:            
            ...
            cursorSQLServer.execute("""UPDATE aro_nexthope_reprise_2 SET nxh_date_nexthope =?   WHERE nxh_ver_ident= ? AND nxh_prm_ident = ? """,(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),id_version_police,id_prime))
            connection.commit()
            print "Add"
            #print "prime"
