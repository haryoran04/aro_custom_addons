...
class connecteur(models.Model):    
    ...
    def update_add_record(self):
        ...
        cursorSQLServer.execute("""SELECT  nxh_date_aro
                                    ...
                                    FROM aro_nexthope_reprise_ap WHERE  nxh_date_nexthope IS NULL """)         
        
               
        resultsSQLAroNextHope = cursorSQLServer.fetchall()    
        print      resultsSQLAroNextHope    
        ...
        if resultsSQLAroNextHope:            
            for field_aro_nexthope in resultsSQLAroNextHope:
                ...
                pol_numpol = field_aro_nexthope[5]
                if typemission in ['AFN','AVT','RES','TER','REV']:
                    ####### TRAITEMENT NORMAL #############
                    self.traitement_normal(id_version_police,id_prime,cursorSQLServer,cursorPOSTGRESOdoo,connection,pol_numpol)
        ...
        if id_version_police:
            ...
            if resultsSQLPolicesId:
                ...
                if resultsSQLFVersionPolices:
                    ...
                    cursorPOSTGRESOdoo.commit()
                ############################ Fin Insertion f_version_police dans odoo ############################################################################################
        
            cursorSQLServer.execute("""UPDATE aro_nexthope_reprise_ap SET nxh_date_nexthope =?   WHERE nxh_ver_ident= ? """,(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),id_version_police))
            connection.commit()
    ...
    def traitement_normal(self,id_version_police,id_prime,cursorSQLServer,cursorPOSTGRESOdoo,connection,pol_numpol):
        print "debut de traitement - Insertion dans la base odoo"
        print id_version_police
        print id_prime
        print "----------------"
        #instance objet dans odoo
        obj_f_polices = self.env['f.polices']
        ...
        if id_version_police:
            ...
            if resultsSQLPolicesId:
                ...
                #Si la police n'existe pas alors on cree dans odoo    
                    ...
                    self.create_police(cursorSQLServer,cursorPOSTGRESOdoo,id_version_police,id_polices,pol_numpol)
                    ############################## ERCIRE F_VERSION_POLICE DANS Odoo ##########################################################
                    cursorSQLServer.execute("""SELECT "VER_DATEFIN",
                                                      "VER_DATEDEB",
                                                      "VER_TYPEMISSION", 
        ...
        if id_version_police and id_prime:            
            ...
            ###############Generate facture ###################################
            self.generate_invoice(id_prime)
            cursorSQLServer.execute("""UPDATE aro_nexthope_reprise_ap SET nxh_date_nexthope =?   WHERE nxh_ver_ident= ? AND nxh_prm_ident = ? """,(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),id_version_police,id_prime))
            connection.commit()
            print "Add"
            #print "prime"
    ...
    def escape_quote_string(self,param):
        if param is None:
            return "''"
        a = param
        return "'" + str(a.replace("'",r"''")) + "'"
    ...
    def create_police(self,cursorSQLServer,cursorPOSTGRESOdoo,id_version_police,id_polices,pol_numpol):
        ...
        if resultsSQLFPolice :                    
            ...
            print POL_COMMENTAIRES
            print self.escape_quote_string(POL_COMMENTAIRES)
            ...
            state = 'brouillon'
            if POL_CODETAT == '1' or POL_CODETAT == '6':
                state = 'encours'
...
#                     print "test"                    
            ...
            cursorPOSTGRESOdoo.execute(""" INSERT INTO f_polices (id,
                                                                  ...
                                                                  "POL_NUMPOL") VALUES (%s,%s,'%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                                                                   ...
                                                                   """ % (POL_IDENT,POL_PTRCONVID_IJ,datetime.datetime.now(),self.none_value(POL_PTRAPID),POL_CODECHPRIN,POL_UNITE_CARENCE,1,
                                                                          ...
                                                                          self.cast_opt_string(state),self.cast_opt_string(POL_JOUR_PRELEVEMENT),self.none_value(POL_PTRPASID), self.escape_quote_string(POL_COMMENTAIRES),self.cast_opt_string(POL_CODINDICE),self.cast_opt_string(POL_COAREP_PRM),
                                                                          ...
                                                                          self.cast_opt_string(POL_CODETAT),datetime.datetime.now(),self.parse_date(POL_DATE_HEURE_EFFET),self.cast_opt_string(POL_CODE_SOCIETE), self.escape_quote_string(POL_ASSURE),self.cast_opt_string(POL_COAREP_SIN),
                                                                          ...
                                                                          self.none_value(POL_COMAPETX),self.escape_quote_string(pol_numpol)))
            cursorPOSTGRESOdoo.commit()
            print "police inserted"

            print "Fin de traitement - Insertion dans la base odoo"
        #####################################################################################################################################################
    ...
    def mouvement_sor(self,cursorSQLServer,cursorPOSTGRESOdoo,id_version_police):
        ...
        if resultsSQLFMouvement:
            ...
            #                 state = 'precedent'
            ##### Adding test ###################
            createMouvement =  'sans'
            if resultsSQLFMouvement[0][3] is not None and resultsSQLFMouvement[0][6] is not None :
                createMouvement = 'creer'
                state = 'suivant'
                
            if resultsSQLFMouvement[0][6] is None and resultsSQLFMouvement[0][3] is not None:
                createMouvement = 'creer'
                state = 'suivant'
                
            if  resultsSQLFMouvement[0][6] is not None and resultsSQLFMouvement[0][3] is None:
                createMouvement = 'misajour'
                MVT_PTRSOR_OBJET = self.none_value(resultsSQLFMouvement[0][6])
                state = 'precedent'
                
            
                
            ##### Adding test for movement: Adding and UPDATE  ##########                                
            ...
            ######################################### Fin insertion et mis a jour f_mouvement f_mouvement #############################################
            ...
            if resultsSQLFSitObjetRisque:
                ...
                if createMouvement == 'creer':
                    ...
                    cursorPOSTGRESOdoo.commit()
                if createMouvement == 'misajour':
                    cursorPOSTGRESOdoo.execute(""" UPDATE f_sit_objet_risque SET "SOR_DATEFIN" = %s, "SOR_DATEFIN_FACT" = %s WHERE id = %s """ % (MVT_DATEDEFFET,MVT_DATEDEFFET,MVT_PTRSOR_OBJET))
    ...
    def mouvement_desc100_clause_garantie(self,cursorSQLServer,cursorPOSTGRESOdoo,id_version_police,SOR_IDENT):        
            ...
            if resultsSQLFdescstat100:
                for i in range(0,len(resultsSQLFdescstat100)):                                          
                    ...
                    name_desc_statadd = '' 
                    if name_desc_stat:
                        name_desc_statadd = name_desc_stat.replace("'", r"''")
                    ...
                    cursorPOSTGRESOdoo.execute(""" INSERT INTO f_desc_stat_100 (    id,
                                              ...
                                              VALUES (%s,'%s',%s,%s,'%s','%s',%s,%s,%s,%s,%s,%s,
                                                      ...
                                                      %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                                                      %s,%s,%s,%s,%s,%s,%s)
                                              """ % (D100_IDENT,datetime.datetime.now(),1,1,datetime.datetime.now(),name_desc_statadd, 
                                                     D100_DESC_00, D100_DESC_01,
...
#                                             
            ...
            if resultsSQLFClausedyn:
                for i in range(0, len(resultsSQLFClausedyn)):
                    ...
                    CLD_TEXTE = self.escape_quote_string(resultsSQLFClausedyn[i][15])
#                     if resultsSQLFClausedyn[i][15] is not None:
#                         Texte = resultsSQLFClausedyn[i][15].replace("'",r"''")
                    ...
                    for i in range(0,len(resultsSQLFPrimeLigne)):                       
                        ...
                        cursorPOSTGRESOdoo.execute(""" INSERT INTO f_prime_ligne (    id,
                                                     ...
                                                     """, (LPR_IDENT,1,LPR_COMAPPCR,LPR_REGREVCR,datetime.datetime.now(),LPR_REVTYPE,LPR_REVRESTE,
                                                           ...
                                                           LPR_PRIMENETTE,LPR_TAXASSMT,LPR_TAXASSTF))
                        cursorPOSTGRESOdoo.commit()



    def generate_invoice(self,id_prime):
        ...
        #### Recuperation du client ############# 
        objPrimeBrowse = self.env['f.prime'].browse([id_prime])

        ##### GET Client #########################
        id_f_p_c_client =  objPrimeBrowse.PRM_VER_IDENT.VER_PTRPOLID.POL_PTRCLID
        res_partner = objPartner.search([('id_f_p_c_client','=',id_f_p_c_client.id)])               
        partner_id = res_partner.id
        partner_final = res_partner.id
        ########## On indique qu'il faut que la facture possède un client ##################        
        if not partner_id:
            #raise ValidationError("Veuillez reseinger le client correspondant")
            return False
        
        ##### Test if apporteur and intermediaire existe #########################
        
        
        id_intermediaire = objPrimeBrowse.PRM_VER_IDENT.VER_PTRPOLID.POL_PTRINID
        id_intermediaire_final = partner_id
        if id_intermediaire:
            id_intermediaire_final = objPartner.search([('id_intermediaire','=',id_intermediaire.id)]).id
            partner_id =   id_intermediaire_final
       
            
                 
        ############################End Apporteur and intermediaire exists ########
        categ_id = objPrimeBrowse.PRM_VER_IDENT.VER_PTRPOLID.POL_PTRPASID.id
                
        #### Recuperation de l'artcile (product) et creationde l'invoice line ###########
        inv_line_values = []        
        if  objPrimeBrowse.PRM_LPRIDENT: 
            for prime_line in objPrimeBrowse.PRM_LPRIDENT:            
...
#                   #############################################
                ...
                if prime_line.LPR_ASSCODCPT == 'FRAIS':
                    ...
                    self.env.cr.execute('SELECT SUM("LPR_FRACIE") FROM f_prime_ligne WHERE "LPR_PTRPRMIDENT" = %s AND "LPR_ASSCODCPT" LIKE %s',(objPrimeBrowse.id,'FRAIS',))
        ...
        inv_values = {  'comment': False, 
                        ...
                        'name': False,                      
                        'amount_total':objPrimeBrowse.PRM_TTCCLIENT,
                        'final_customer_id':partner_final,
                        }
        ...
        #########################Set is_invoiced 1#######################################   
        #objPrimeBrowse.write({'is_invoiced':True,'invoice_id':invoice_ids.id})
        #self.pool.get('f.prime').write(self.env.cr,1, [objPrimeBrowse.id], {'is_invoiced':True,'invoice_id':invoice_ids.id}, context ={})        
